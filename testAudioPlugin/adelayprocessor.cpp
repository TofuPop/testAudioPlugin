//yadayada here we go. Steinberg comments marked with [Stein]
...
//using namespace Steinberg; we don't need these here because we used them in the header file.
...
namespace Vst
{
	...
	ADelayProcessor::ADelayProcessor() // probably erroring because we need the header file. So I'll go do that now. Nope, gonna figure it out eventually. It was a typo in the file name.
		: mDelay(1)
		, mBuffer(0)
		, mBufferPos(0)
		, mBypass(false)
	...
	{
		// we only support one in and output bus and these buses must have the same number of channels [Stein]
	...
	tresult PLUGIN_API ADelayProcessor::setActive(TBool state)
	{
		...
		if (state)
		{
			...
			{
				mBuffer[channel] = (float*)std::malloc(size); //1 sec delay max [Stein]
